...
<xsl:template match="page" mode="good">
	...
	<Товар>
		...
		<ЗначенияСвойств>
			<xsl:apply-templates select="key('type', @type-id)/fieldgroups/group[not(@name = 'common' or @name = 'cenovye_svojstva' or @name = 'product' or @name = 'menu_view' or @name = 'more_params' or @name = 'rate_voters' or @name = 'locks' or @name = 'catalog_stores_props')]/field[not(@name = 'photo' or @name='description')]"  mode="good-property">
		...
		<xsl:choose>
			<xsl:when test="type/@data-type = 'relation' and count($prop-value/item)">
				<xsl:for-each select="$prop-value/item">
					<ИдЗначения><xsl:value-of select="$prop-value/item/@id" /></ИдЗначения>
					<Значение><xsl:value-of select="@name" /></Значение>
				</xsl:for-each>
			</xsl:when>
...
<xsl:template match="field" mode="property">
	<xsl:param name="type_id" select="@guide-id"/>
	<xsl:if test="generate-id(.) = generate-id(key('property', ./@id))">
		<Свойство>
			<Ид><xsl:value-of select="@id" /></Ид>
			<Наименование><xsl:value-of select="@title" /></Наименование>
			<Имя><xsl:value-of select="@name" /></Имя>
			<ТипыЗначений>
				<ТипЗначений>
					<Тип>
						<xsl:choose>
							<xsl:when test="type/@data-type = 'relation'">Справочник</xsl:when>
					...
					<xsl:if test="type/@data-type = 'relation'">
						<ВариантыЗначений>
							<xsl:apply-templates select="../../../../../pages/page/properties/group/property[@type = 'relation']/value/item[@type-id = $type_id]" mode="relation-value" />
						</ВариантыЗначений>
					</xsl:if>
				</ТипЗначений>
			</ТипыЗначений>
...
<xsl:template match="item" mode="relation-value">
	<ВариантЗначения>
		<Ид><xsl:value-of select="@id" /></Ид>
		<Значение><xsl:value-of select="@name" /></Значение>
	</ВариантЗначения>

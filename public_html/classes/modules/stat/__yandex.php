	...
	abstract class __yandex_stat extends baseModuleAdmin {
		...
		public function edit_counter($counterId = false) {
			...
			if($mode == "do") {
				...
				secure_load_dom_document($response, $dom);
		...
		public function view_counter_json() {
			...
			$cacheDir = SYS_TEMP_PATH . '/metrika/';
			...
			if (file_exists($cacheFile) && $session->get('metrikaKey') == $key) {
			...
			if (is_null($filteredResponse)) {
				...
				$headers = array ("Content-type" => 'application/json', "Accept" => 'application/x-yametrika+json');

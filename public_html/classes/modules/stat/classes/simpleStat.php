...
abstract class simpleStat
{
    ...
    */
	...
	public function setDomain($v_domain) {		
		...
		if ($i_domain === -1 || strtoupper($s_domain) === "ALL" || $s_domain == "Все") {
			$this->host_id = -1;
		} elseif ($i_domain && (strval($i_domain) === strval($v_domain))) {
			$o_domains = domainsCollection::getInstance();
			$o_dom = $o_domains->getDomain($i_domain);
			if ($o_dom) {
				$s_query = "SELECT   `group_id` AS 'id' FROM `cms_stat_sites` WHERE `name` = '".$connection->escape($o_dom->getHost())."'";
				$queryResult = $connection->queryResult($s_query);
				$queryResult->setFetchType(IQueryResult::FETCH_ROW);
				$row = $queryResult->fetch();
				$this->host_id = intval($row['id']);
			}
		} else {
	...
	public function setCmsDomainsArray($arr_domains = array()) {
		...
		foreach($arr_domains as $i_dom_id) {
			...
			if ($o_dom) $arr_tmp[] = $connection->escape($o_dom->getHost());
		}

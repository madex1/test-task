...
class refuses extends simpleStat
{
    ...
    public function get()
    {
		...
		$connection->query("DROP TEMPORARY TABLE IF EXISTS `tmp_pages_refuse`");
		$connection->query("CREATE TEMPORARY TABLE `tmp_pages_refuse` (`path_id` INT, `level` INT, KEY `path_id_level` (`path_id`, `level`)) ENGINE = MEMORY");
		$connection->query("INSERT INTO `tmp_pages_refuse` SELECT `path_id`, MAX(`number_in_path`) AS `mnum` FROM `cms_stat_hits` `h`
                         INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                          WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " " . $this->getHostSQL("p") . $this->getUserFilterWhere('p') . "
                           GROUP BY `path_id`
                            HAVING `mnum` = 1");
		$connection->query("SET @all_refuses = (SELECT COUNT(*) FROM tmp_pages_refuse)");
		$connection->query("SET @all_visits = (SELECT COUNT(*) FROM `cms_stat_paths` `p`

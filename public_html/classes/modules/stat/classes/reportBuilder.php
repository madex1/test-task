...
class reportBuilder
{
    ...
    public function doQuery($index)
    {
        ...
        if (empty($this->filters)) {
			...
			$connection->query($qry1 = "DROP TEMPORARY TABLE IF EXISTS `" . $tmp_table . "`");
			$connection->query($qry2 = "CREATE TEMPORARY TABLE `" . $tmp_table . "` (`id` INT, KEY `id` (`id`)) ENGINE = MEMORY");
			$connection->query($qry3 = "INSERT INTO `" . $tmp_table . "` " . $pre . " WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " " . $this->getHostSQL("p") . "");
        }
        ...
        foreach ($this->filters as $key => $val) {
            ...
            if (is_array($val['value'])) {
                foreach ($val['value'] as $v) {
                    $values .= "'" . $connection->escape($v) . "', ";
                }
                $values = substr($values, 0, -2);
            } else {
                $values = "'" . $connection->escape($val['value']) . "'";
            }

			$connection->query("DROP TEMPORARY TABLE IF EXISTS `" . $tmp_table . "`");
			$connection->query("CREATE TEMPORARY TABLE `" . $tmp_table . "` (`id` INT, KEY `id` (`id`)) ENGINE = MEMORY");
			...
			$connection->query($qry) or die($connection->errorDescription());
        }
    ...
    {
		$connection = ConnectionPool::getInstance()->getConnection();
        $queryResult = $connection->queryResult("SELECT `group_id` AS `id` FROM `cms_stat_sites`
                             WHERE `name` = '" . $connection->escape($hostname) . "'");
		$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);

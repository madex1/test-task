...
class pageNext extends simpleStat
{
    ...
    public function get()
    {
		...
		if(!$this->params['page_uri']) {
			...
			$sql = "SELECT id FROM cms_stat_pages WHERE uri = '" . $connection->escape($this->params['page_uri']) . "' " . $this->getHostSQL("") . " LIMIT 1";
			$result = $connection->queryResult($sql);
			$result->setFetchType(IQueryResult::FETCH_ROW);
			$page_id = ($result->length() > 0) ? array_shift($result->fetch()) : null;
		}

...
class statistic {
	...
	public function __construct($time = null) {
		...
		if (!$cookieJar->isExists('stat_id')) {
			$cookieJar->set('stat_id', $this->getSessionId(), strtotime('+10 years'));
		}
	}
	...
	public function run() {
		...
		if ($this->getSessionStatByKey('isSearchBot') == true) {
			return false;
		}
		...
		if ($this->issetSessionStatByKey('doLogin')) {
			...
			if ($this->issetSessionStatByKey('loginId')) {
				...
				$qry = "SELECT `id` FROM `cms_stat_users` WHERE `login` = '" . $connection->escape($login) . "' AND `host_id` = " . $this->getSessionStatByKey('site_id');
				...
				if ($this->issetSessionStatByKey('path_id')) {
					$qry = "UPDATE `cms_stat_paths` SET `user_id` = " . $user_id . " WHERE `id` = " . $this->getSessionStatByKey('path_id');
		...
		if (!$this->issetSessionStatByKey('id')) {
			...
			if (!$cookieJar->isExists('stat_id')) {
				...
				$cookieJar->set('stat_id', $this->getSessionId(), time() + 315360000);
				...
				$queryResult = $connection->queryResult("SELECT `id` FROM `cms_stat_users` WHERE `session_id` = '" . $sessid . "' AND `login`='{$login}'");
				$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
				$row = $queryResult->fetch();
				...
				if (isset($row['id'])) {
					...
					if (isset($row['ts']) && ($this->time - $row['ts']) / 60 <= STAT_RETURN_INTERVAL) {
						...
						$queryResult = $connection->queryResult("SELECT COUNT(*) AS `cnt` FROM `cms_stat_hits` WHERE `path_id` = " . $row['id']);
						$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
						$row = $queryResult->fetch();
						...
						if (isset($arr[$row['varname']])) {
							...
							$qry = "SELECT `s`.`id` FROM `cms_stat_sources_search_queries` `q`
										 INNER JOIN `cms_stat_sources_search` `s` ON `s`.`text_id` = `q`.`id`
										  WHERE `q`.`text` = '" . $connection->escape($text) . "' AND `engine_id` = " . $engine_id;
							$queryResult = $connection->queryResult($qry);
							$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
							$row = $queryResult->fetch();
							// если таковые есть
							if (isset($row['id'])) {
								...
								$qry = "SELECT `id` FROM `cms_stat_sources_search_queries` WHERE `text` = '" . $connection->escape($text) . "'";
								$queryResult = $connection->queryResult($qry);
								$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
								$row = $queryResult->fetch();
								if (isset($row['id'])) {
									...
									$qry = "INSERT INTO `cms_stat_sources_search_queries` (`text`) VALUES ('" . $connection->escape($text) . "')";
									$connection->query($qry);
									$word_id = $connection->insertId();
					...
					if (!isset($row['group_id']) || $row['group_id'] != $this->getSessionStatByKey('site_id')) {
						...
						$qry = "SELECT `s`.`id` FROM `cms_stat_sources_sites_domains` `d`
								 INNER JOIN `cms_stat_sources_sites` `s` ON `s`.`domain` = `d`.`id`
								  WHERE `d`.`name` = '" . $connection->escape($domain) . "' AND `s`.`uri`='".$connection->escape($uri_ref)."'";
						$queryResult = $connection->queryResult($qry);
						$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
						$row = $queryResult->fetch();
						...
						if (isset($row['id'])) {
							...
							if (isset($row['id'])) {
								...
								$qry = "INSERT INTO `cms_stat_sources_sites` (`uri`, `domain`) VALUES ('" . $connection->escape($uri_ref) . "', " . $row['id'] . ")";
								$connection->query($qry);
								$source_id = $connection->insertId();
							} else {
								// если домена нет - то добавляем домен
								$qry = "INSERT INTO `cms_stat_sources_sites_domains` (`name`) VALUES ('" . $connection->escape($domain) . "')";
								$connection->query($qry);
								$rel_site_id = $connection->insertId();

								$qry = "INSERT INTO `cms_stat_sources_sites` (`uri`, `domain`) VALUES ('" . $connection->escape($uri_ref) . "', " . $rel_site_id . ")";
								$connection->query($qry);
								$source_id = $connection->insertId();
							}
						}
			...
			if (!$this->issetSessionStatByKey('path_id') || true) {
				// установка path_id
				$qry = "INSERT INTO `cms_stat_paths` (`user_id`, `date`, `host_id`" . (isset($source_id) ? ", `source_id`" : "") . ") VALUES (" . $this->getSessionStatByKey('user_id') . ", '" . $this->getNow() . "', " . $this->getSessionStatByKey('site_id') . "" . (isset($source_id) ? ", " . $source_id : "") . ")";
				$connection->query($qry);
		...
		if (!$source_id && isset($_GET['_openstat'])) {
			...
			try {
				...
				$qry = "INSERT INTO `cms_stat_sources_openstat` (`service_id`, `campaign_id`, `ad_id`, `source_id`, `path_id`)
							 VALUES (" . $openstat->getServiceId()  . ", " . $openstat->getCampaignId() . ", " . $openstat->getAdId() . ", " . $openstat->getSourceId() . ", " . $this->getSessionStatByKey('path_id') . ")";
				$connection->query($qry);
			} catch (Exception $e) {
				echo $e->getMessage();
			}
		...
		$qry = "SELECT `id` FROM `cms_stat_pages` WHERE `uri` = '" . $uri . "' AND `host_id` = " . $this->getSessionStatByKey('site_id');
		$queryResult = $connection->queryResult($qry);
		$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
		$row = $queryResult->fetch();
		if (isset($row['id'])) {
			...
			if (strrpos($this->getUri(), '/') === 0) {
				...
				$section = $connection->escape(substr($this->getUri(), 1, strpos($this->getUri(), '/', 1) - 1));
			}

			$qry = "INSERT INTO `cms_stat_pages` (`uri`, `host_id`, `section`) VALUES ('" . $uri . "', '" . $this->getSessionStatByKey('site_id') . "', '" . $section . "')";
			$connection->query($qry);
		...
		$anotherPrevPageId = ($this->issetSessionStatByKey('prev_page_id')) ? ", " . $this->getSessionStatByKey('prev_page_id') : '';

		$qry = "INSERT INTO `cms_stat_hits` (`page_id`, `date`, `hour`, `day_of_week`, `week`, `day`, `month`, `year`, `path_id`, `number_in_path`" . $prevPageId . ") VALUES
					 (" . $page_id . ", '" . $this->getNow() . "', HOUR('" . $this->getNow() . "'), DATE_FORMAT('" . $this->getNow() . "', '%w'), WEEK('" . $this->getNow() . "'), DAY('" . $this->getNow() . "'), MONTH('" . $this->getNow() . "'), YEAR('" . $this->getNow() . "'), " . $this->getSessionStatByKey('path_id') . ", " . $this->getSessionStatByKey('number_in_path') . $anotherPrevPageId . ")";
		$connection->query($qry);
	...
	public function entry($name) {
		...
		$qry = "SELECT   `p`.`url`, `e`.`event_id` FROM `cms_stat_entry_points` `p`
					 LEFT JOIN `cms_stat_entry_points_events` `e` ON `e`.`entry_point_id` = `p`.`id`
					  WHERE `name` = '" . $connection->escape($name) . "' AND `host_id` = " . $this->getSessionStatByKey('site_id');
	...
	public function event($name) {
		...
		$qry = "SELECT `id` FROM `cms_stat_events`
					 WHERE `name` = '" . $name . "' AND `host_id` = " . $this->getSessionStatByKey('site_id');
		...
		if (is_array($row) && isset($row['id'])) {
			...
			$qry = "INSERT INTO `cms_stat_events` (`description`, `name`, `type`, `profit`, `host_id`) VALUES ('" . $name . "', '" . $name . "', 2, 0, " .$this->getSessionStatByKey('site_id') . ")";
			$connection->query($qry);
	...
	/**
	 * Возращает HTTP_HOST, если его нет - SERVER_NAME
	 * @return string
	 */
	...
	private function setUpHostId() {
		if (!$this->issetSessionStatByKey('site_id')) {
			$connection = ConnectionPool::getInstance()->getConnection();
			$name = $connection->escape($this->getServerName());
	...
	private function addUser() {
		...
		$qry = "INSERT INTO `cms_stat_users` (`session_id`, `first_visit`, `login`, `os_id`, `browser_id`, `ip`, `location`, `js_version`, `host_id`) VALUES
					('" . $this->getSessionId() . "', '" . $this->getNow() . "', '" . $login . "', '" . (int) $this->getOsId() . "', '" . (int)$this->getBrowserId() . "', '" . $connection->escape($this->getRemoteAddr()) . "', '" . $this->getLocation($this->getRemoteAddr()) . "', '" . $this->getJsVersion() . "', " . $this->getSessionStatByKey('site_id') . ")";
		$connection->query($qry);
	...
	private function getBrowserId() {
		$connection = ConnectionPool::getInstance()->getConnection();
		$browser = $connection->escape(UmiCms\Service::Request()->getBrowser());
	...
	private function getOsId() {
		$connection = ConnectionPool::getInstance()->getConnection();
		$os = $connection->escape(UmiCms\Service::Request()->getPlatform());
	...
	private function getLocation($ip) {
		...
		if ($oGeoMod === false) {
			...
			if (!isset($aIPInfo['city']) && !isset($aIPInfo['region']) && !isset($aIPInfo['country'])) {
				return "";
			} else {
				return $aIPInfo['city'] . ', ' . $aIPInfo['region'] . ', ' . $aIPInfo['country'];
			}
		}
	}
	...
	private function isSearchBot() {
		if (UmiCms\Service::Request()->isRobot()) {
			return true;
		}
	...
	private function getSessionId() {
		return \UmiCms\Service::Session()->getId();
	}
	...
	private function getSessionStatByKey($key) {
		$session = \UmiCms\Service::Session();
		$statData = $this->getSessionStat($session);
		$value = (isset($statData[$key])) ? $statData[$key] : null;
		return $value;
	}

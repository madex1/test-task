<?php
	abstract class __subscribers_import_dispatches extends baseModuleAdmin {
		...
		public function subscribers_import() {
			...
			$params = Array(
				'options' => Array(
						'file:csvfile' => Array('value' => NULL, 'destination-folder' => USER_FILES_PATH)
					)
			);
		...
		public function subscribers_import_do() {
			...
			if(!($csvfile = umiFile::upload("data", "csvfile", USER_FILES_PATH ))) $csvfile = new umiFile(USER_FILES_PATH . $select_csvfile);

	...
	abstract class __releasees_releasees {
		...
		public function getNewReleaseInstanceId($iDispId = false) {
			...
			if (isset($arrReleasees[$iDispId])) {
				...
				$iReleaseTypeId =  umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeId($iHierarchyTypeId);
		...
		public function fill_release($dispatch_id = false, $ignore_redirect = false) {
			...
			if ($oDispatch instanceof umiObject) {
				...
				if (count($arrNewsLents)) {
					...
					$object_type_id = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("news", "item");
...
*/
		...
		public function release_send_full($iDispId) {
			...
			foreach($sel->result() as $recipient) {
				...
				/** @var __releasees_releasees|dispatches $this */
		...
		public function release_send() {
			...
			$progressKey = 'umi_send_list_' . $iReleaseId;
			...
			if (!$session->isExist($progressKey)) {
				...
				$session->set($progressKey, $arrRecipients);
				$session->set($progressCountKey, count($arrRecipients));
			}
			else $arrRecipients = $session->get($progressKey);

			$delay = $session->get($progressDelayKey);
			$iTotal = (int) $session->get($progressCountKey);
...
END;
			...
			if ($sel->length()) {
				...
				$session->del($iDispId . '_new_templater');
			...
			foreach($arrRecipients as $recipient_id) {
				...
				if ($delay) {
					...
					$session->set($progressKey, array_diff($arrRecipients, $aSended));
					$iTotal = (int) $session->get($progressCountKey);
...
END;
			...
			if (!count($session->get('umi_send_list_' . $iReleaseId))) {
				$oRelease->setValue('status', true);
				$oDate = new umiDate(time());
				$oRelease->setValue('date', $oDate);
				$oRelease->setName($oDate->getFormattedDate('d-m-Y H:i'));
			...
			$iTotal = (int) $session->get('umi_send_list_' . $iReleaseId . '_count');
			$iSended = $iTotal - (count($arrRecipients) - count($aSended));
...
END;
		...
		public function collectAllChanges($module, $method, $parentElementId = false) {
				...
				$object_type_id = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName($module, $method);
				$object_type = umiObjectTypesCollection::getInstance()->getType($object_type_id);
				$publish_time_field_id = $object_type->getFieldId('publish_time');
			...
			if(!is_numeric($parentElementId)) {
				...
				foreach($result as $elementId) {
						$childs = count(umiHierarchy::getInstance()->getChildrenTree($elementId));

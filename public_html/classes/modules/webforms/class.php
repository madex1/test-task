...
class webforms extends def_module {
	...
	public function __construct() {
		...
		if(cmsController::getInstance()->getCurrentMode() == "admin") {
			...
			$this->__implement("__webforms_custom_admin");
		}
	...
	public function getBindedPage($formId = false) {
		...
		$typeId = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeId($hType->getId());
	...
	public function formatMessage($message, $b_split_long_mode = 0) {
		$args = func_get_args();
		$_iMessageId = array_shift($args);

		if (!is_numeric($_iMessageId)) {
			throw new Exception('Message id expected for format message');
		}

		$_bProcessAll = array_shift($args);
		$_bProcessAll = (is_null($_bProcessAll)) ? false : $_bProcessAll;

		$oObjects    = umiObjectsCollection::getInstance();
		$iTplTypeId  = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName('webforms', 'template');
	...
	public function checkAddressExistence($_sAddress) {
		...
		$iTypeId    = $oTypes->getTypeIdByHierarchyTypeName('webforms', 'address');
		$oSelection = new umiSelection;
	...
	public function checkRequiredFields($typeId) {
		return cmsController::getInstance()->getModule('data')->checkRequiredFields($typeId);
	}

	public function assembleErrorFields($errorFields) {
		return cmsController::getInstance()->getModule('data')->assembleErrorFields($errorFields);
	}
	...
	private function writeAddressSelect($template, $iFormId = false) {
		...
		foreach($addresses as $address) {
			...
			$aItems[] = self::parseTemplate($line, $aParam, false, $addressId);
		}
		$aBlockParam = array();
	...
	public function add($form_id = false, $who = "", $template="webforms") {
		...
		if(!is_numeric($form_id) || $form_id === false) {
			$aChilds = $oTypes->getChildTypeIds($iBaseTypeId);
			if(empty($aChilds)) {
...
*/
	public function insert($who = "", $template = "default") {
		...
		$oObjects   = umiObjectsCollection::getInstance();
		$oSelection = new umiSelection;
		$oSelection->addObjectType( umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName('webforms', 'address') );
	...
	public function post() {
		...
		if(is_numeric($email_to)) {
			...
			if(intval($to) != $email_to) {
				...
				$iTypeId      = $oTCollection->getTypeIdByHierarchyTypeName('webforms', 'address');
		...
		$someMail->setFrom($from_email, $from);
...
END;
			...
			$max_size = getBytesFromString(mainConfiguration::getInstance()->get('system', 'quota-uploads'));
			if ($max_size != 0) {
				$summary_size = getBusyDiskSize(getUploadsDir());
			}
...
END;
		...
		if($template = (string) $_REQUEST['template']) {    //Sending auto-reply
			...
			$template_mail = def_module::parseTemplateForMail($template_mail, array());
			$template_mail_subject = def_module::parseTemplateForMail($template_mail, array());
			$check_param = false;
	...
	public function getUnbindedForms($currentTemplateId = false) {
		...
		$typeId    = $typesCollection->getTypeIdByHierarchyTypeName('webforms', 'template');
	...
	public function getObjectEditLink($objectId, $type = false) {
		...
		if($oType->getParentId() == umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("webforms", "form")) $type = "message";
		switch($type) {
			case 'form'     :

...
*/
	class orderItem extends umiObjectProxy {
		...
		/** @var string ITEM_DISCOUNT_VALUE_FIELD_GUID гуид поля объекта-источника для наименования, в котором хранится абсолютное значение скидки наименования */
		...
		*/
		public function getItemElement($dontDelete = false) {
		...
		*/
		...
		public function setDiscountValue($value) {
			$value = (float) $value;
			$orderItemPrice = $this->getItemPrice();

			if ($value > $orderItemPrice) {
		...
		*/
		...
		protected function getElementPrice() {
			...
			if(count($element) && $element[0] instanceof iUmiHierarchyElement) {
				$emarket = cmsController::getInstance()->getModule('emarket');
		...
		private static function getItemTypeId($objectTypeId) {
			...
			} else return $cache[$objectTypeId] = null;
		}

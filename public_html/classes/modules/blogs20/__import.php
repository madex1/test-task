...
abstract class __import {	
	public function import_old_blogs() {
		...
		$blog20Type  		= $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blog20HType->getId()) );
		$blog20PostHType 	= $hierarchyTypesCollection->getTypeByName('blogs20', 'post');
		$blog20PostType  	= $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blog20PostHType->getId()) );
		$blog20CommentHType = $hierarchyTypesCollection->getTypeByName('blogs20', 'comment');
		$blog20CommentType  = $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blog20CommentHType->getId()) );
		$blogHType   		= $hierarchyTypesCollection->getTypeByName('blogs', 'blog');
		$blogType    		= $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blogHType->getId()) );
		$blogPostHType   	= $hierarchyTypesCollection->getTypeByName('blogs', 'blog_message');
		$blogPostType    	= $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blogHType->getId()) );
		$blogCommentHType   = $hierarchyTypesCollection->getTypeByName('blogs', 'blog_comment');
		$blogCommentType    = $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($blogHType->getId()) );		
	...
	public function makeElementFromExisting($sourceId, $parentId, $newHTypeId, $additionalCopyPairs = array()) {		
		...
		if(!($oldPrivacyFriends && $oldPrivacyOwner)) {
			$privacyType   = $typesCollection->getTypeIdByHierarchyTypeName('blogs', 'blog_privacy');
		...
		$newType    = $typesCollection->getType( $typesCollection->getTypeIdByHierarchyTypeId($newHTypeId) );

...
class SphinxClient
{
	...
	function _Connect ()
	{
		...
		// check version
		list($umi_temp,$v) = unpack ( "N*", fread ( $fp, 4 ) );
		$v = (int)$v;
		if ( $v<1 )
		{
			fclose ( $fp );
	...
	function _GetResponse ( $fp, $client_ver )
	{
		...
		{
			list($umi_temp,$wlen) = unpack ( "N*", substr ( $response, 0, 4 ) );
	...
	function _PackFloat ( $f )
	{
		...
		list($umi_temp,$t2) = unpack ( "L*", $t1 ); // int in machine order
		return pack ( "N", $t2 );
	}
	...
	function _ParseSearchResponse ( $response, $nreqs )
	{
		...
		for ( $ires=0; $ires<$nreqs && $p<$max; $ires++ )
		{
			...
			// extract status
			list($umi_temp,$status) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			$result["status"] = $status;
			if ( $status!=SEARCHD_OK )
			{
				list($umi_temp,$len) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			...
			list($umi_temp,$nfields) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			...
			list($umi_temp,$nattrs) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			while ( $nattrs-->0 && $p<$max  )
			{
				list($umi_temp,$len) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
				$attr = substr ( $response, $p, $len ); $p += $len;
				list($umi_temp,$type) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
				$attrs[$attr] = $type;
			}
			$result["attrs"] = $attrs;

			// read match count
			list($umi_temp,$count) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			list($umi_temp,$id64) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
			...
			while ( $count-->0 && $p<$max )
			{
				...
				if ( $id64 )
				{
					...
					list($umi_temp,$weight) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
				}
				else
				{
				...
				foreach ( $attrs as $attr=>$type )
				{
					...
					{
						list($umi_temp,$uval) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
						list($umi_temp,$fval) = unpack ( "f*", pack ( "L", $uval ) ); 
						$attrvals[$attr] = $fval;
						continue;
					}

					// handle everything else as unsigned ints
					list($umi_temp,$val) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
	...
	function BuildExcerpts ( $docs, $index, $words, $opts=array() )
	{
		...
		{
			list($umi_temp,$len) = unpack ( "N*", substr ( $response, $pos, 4 ) );
			$pos += 4;
	...
	function BuildKeywords ( $query, $index, $hits )
	{
		...
		$res = array ();
		$rlen = strlen($response);
		list($umi_temp,$nwords) = unpack ( "N*", substr ( $response, $pos, 4 ) );
		$pos += 4;
		for ( $i=0; $i<$nwords; $i++ )
		{
			list($umi_temp,$len) = unpack ( "N*", substr ( $response, $pos, 4 ) );	$pos += 4;
	...
	function UpdateAttributes ( $index, $attrs, $values, $mva=false )
	{
		...
		// parse response
		list($umi_temp,$updated) = unpack ( "N*", substr ( $response, 0, 4 ) );
		return $updated;
	}
	...
	function FlushAttrs ()
	{
		...
		if ( strlen($response)==4 )
			list($umi_temp,$tag) = unpack ( "N*", $response );

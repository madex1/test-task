<?php
	abstract class __forget_users {
		...
		public function forget_do($template = "default") {
			...
			if ($user_id) {
				...
				if ($without_act || intval($object->getValue('is_activated'))) {
					...
					$mail_arr['restore_link'] = $domain->getCurrentUrl() . $this->pre_lang . "/users/restore/" . $activate_code . "/";
		...
		public function restore($activate_code = false, $template = "default") {
			...
			$object_type_id = umiObjectTypesCollection::getInstance()->getTypeIdByHierarchyTypeName("users", "user");

<?php
	 ...
	 * @link http://help.docs.umi-cms.ru/rabota_s_modulyami/module_tickets/
	 */
	class tickets extends def_module {
		...
		public function includeAdminClasses() {
			...
			$this->__implement('TicketsAdmin');

			$this->loadAdminExtension();

			$this->__loadLib('customAdmin.php');
			$this->__implement('TicketsCustomAdmin', true);
		...
		public function includeCommonClasses() {
			...
			$this->__implement('TicketsHandlers');

			$this->__loadLib('customMacros.php');
			$this->__implement('TicketsCustomMacros', true);
		...
		public function manage() {
			...
			if ($id) {
				...
				$this->validateEntityByTypes($ticket, ['module' => 'content', 'method' => 'ticket']);
			} else {
			...
			if ($mode == 'create') {
				$ticket->setValue('create_time', strtotime(date('d.m.Y H:i:00')));
			}
			...
			$url = getRequest('referer') ?: getServer('HTTP_REFERER');
			...
			if ($mode == 'create') {
				$auth = Service::Auth();
				$ticket->setValue('user_id', $auth->getUserId());
			}

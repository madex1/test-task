<?php
	...
	class TrashAdmin {
		...
		public function trash() {
			/** следующие два параметра нужно брать через getRequest() из-за манипулиций с сессией в этой функции */
		...
		private function redirectToModuleIfNeeds() {
			if ($this->isNeedRedirect()) {
				$this->chooseRedirect($this->module->pre_lang . '/admin/trash/trash/');
			}
		}

		/**
		 * Определяет необходимо ли редирект
		 * @return bool
		 * @throws expectElementException
		 */

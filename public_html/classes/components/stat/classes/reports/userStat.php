<?php
	...
	/** Класс получения отчета о пользователе статистики */
	class userStat extends simpleStat {
		...
		public function get() {
			...
			if ((int) $this->params['user_id']) {
				...
				$connection->query('SET @visits_count = (SELECT COUNT(*) AS `visits_cnt` FROM `cms_stat_paths` `p`
				...
				$connection->query('SET @first_path = (SELECT MIN(`id`) FROM `cms_stat_paths` WHERE `user_id` = ' .
											 ...
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_sites` `ss` ON `s`.`concrete_src_id` = `ss`.`id`
											   INNER JOIN `cms_stat_sources_sites_domains` `d` ON `d`.`id` = `ss`.`domain`
												WHERE `s`.`src_type` = 1 AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . $this->getUserFilterWhere('p') . "
												 )
											 ...
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_search` `ss` ON `ss`.`id` = `s`.`concrete_src_id`
											   INNER JOIN `cms_stat_sources_search_engines` `sse` ON `sse`.`id` = `ss`.`engine_id`
												WHERE `s`.`src_type` = 2  AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . $this->getUserFilterWhere('p') . "
												 )
											...
											SELECT 'pr' AS `type`, `pr`.`name` AS `name`  FROM `cms_stat_paths` `p`
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_pr` `pr` ON `pr`.`id` = `s`.`concrete_src_id`
											   WHERE `s`.`src_type` = 3 AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . $this->getUserFilterWhere('p') . "
												)
											...
											SELECT 'ticket' AS `type`, `t`.`name` AS `name` FROM `cms_stat_paths` `p`
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_ticket` `t` ON `t`.`id` = `s`.`concrete_src_id`
											   WHERE `s`.`src_type` = 4 AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . $this->getUserFilterWhere('p') . "
												)
											...
											SELECT 'coupon' AS `type`, `c`.`descript` AS `name` FROM `cms_stat_paths` `p`
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_coupon` `c` ON `c`.`id` = `s`.`concrete_src_id`
											   WHERE `s`.`src_type` = 5 AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . $this->getUserFilterWhere('p') . "
												)
				...
				$tmp = $this->simpleQuery('SELECT  ( UNIX_TIMESTAMP(MAX(`date`)) - UNIX_TIMESTAMP(MIN(`date`)) ) / (COUNT(*) - 1) / 3600 / 24  AS `days` FROM `cms_stat_paths`
											 WHERE `date` BETWEEN ' . $this->getQueryInterval() . ' AND ' . $this->getHostSQL() .
					' AND `user_id` = ' . (int) $this->params['user_id']);
				...
				$tmp = $this->simpleQuery('SELECT COUNT(*) AS `abs`, COUNT(*) / @visits_count * 100 AS `rel`  FROM `cms_stat_hits` `h`
					...
					' AND `h`.`number_in_path` = 1 AND `h2`.`id` IS NULL AND `p`.`user_id` = ' . (int) $this->params['user_id'] .
					'  ' . $sHostSQL);
				$result['refuse_frequency'] = $tmp[0];
				unset($tmp);

				$connection->query('SET @hits_count = (SELECT COUNT(*) FROM `cms_stat_hits` `h`
							 INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
							  INNER JOIN `cms_stat_pages` `pg` ON `pg`.`id` = `h`.`page_id`
							   WHERE `h`.`date` BETWEEN ' . $this->getQueryInterval() . ' AND `p`.`user_id` = ' .
					(int) $this->params['user_id'] . ' ' . $sHostSQL . ')');
					...
					$tmp = $this->simpleQuery("(SELECT 'sites' AS `type`, `d`.`name` AS `name` FROM `cms_stat_paths` `p`
											 INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
											  INNER JOIN `cms_stat_sources_sites` `ss` ON `s`.`concrete_src_id` = `ss`.`id`
											   INNER JOIN `cms_stat_sources_sites_domains` `d` ON `d`.`id` = `ss`.`domain`
												WHERE `s`.`src_type` = 1 AND `s`.`src_type` = " . (int) $tmp['src_type'] .
						' AND `s`.`concrete_src_id` = ' . (int) $tmp['concrete_src_id'] . "
												 )
				...
				$connection->query('SET @last_path = (SELECT `p`.`id` FROM `cms_stat_paths` `p`
							 WHERE `p`.`user_id` = ' . (int) $this->params['user_id'] . '  ' . $sHostSQL . '
							  ORDER BY `p`.`id` DESC
							   LIMIT 1)');
				...
				$connection->query('DROP TEMPORARY TABLE IF EXISTS `tmp_collected_events`');
				$connection->query('CREATE TEMPORARY TABLE `tmp_collected_events` (`id` INT, `name` CHAR(255), `description` CHAR(255), `profit` float(9,2), KEY `id` (`id`)) ENGINE = MEMORY');

				$connection->query('INSERT INTO `tmp_collected_events` SELECT IFNULL(`e2`.`id`, `e`.`id`), IFNULL(`e2`.`name`, `e`.`name`), IFNULL(`e2`.`description`, `e`.`description`), IFNULL(`e2`.`profit`, `e`.`profit`) FROM `cms_stat_hits` `h`
								 INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
								   ...
								   INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
									LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
									 LEFT JOIN `cms_stat_events` `e2` ON `e2`.`id` = `er`.`event_id`
									  WHERE `p`.`date` BETWEEN ' . $this->getQueryInterval() . ' AND `p`.`user_id` = ' .
					(int) $this->params['user_id'] . ' AND `e`.`type` = 1  ' . $sHostSQL);
								  ...
								  INNER JOIN `cms_stat_events_collected` `ec` ON `ec`.`hit_id` = `h`.`id`
								   INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
									LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
									 LEFT JOIN `cms_stat_events` `e2` ON `e2`.`id` = `er`.`event_id`
									  WHERE `p`.`date` BETWEEN ' . $this->getQueryInterval() . ' AND `p`.`user_id` = ' .
					(int) $this->params['user_id'] . ' AND `e`.`type` = 2');

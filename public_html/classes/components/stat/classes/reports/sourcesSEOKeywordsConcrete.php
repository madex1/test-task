<?php
	...
	/** Класс получения отчета о числе запросов с разных поисковиков по конкретному слову */
	class sourcesSEOKeywordsConcrete extends simpleStat {

		/** @inheritdoc */
		protected $params = ['query_id' => 0];
		...
		public function get() {
			$resSumm = $this->simpleQuery('SELECT COUNT(*) AS `cnt` FROM `cms_stat_sources` `s`
				...
				$this->getUserFilterWhere('p') . ' AND `ssq`.`id` = ' . (int) $this->params['query_id'] . ' ' .
				$this->getHostSQL('p') . '
											ORDER BY `cnt` DESC');

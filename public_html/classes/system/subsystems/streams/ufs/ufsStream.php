<?php
	...
	class ufsStream extends umiBaseStream {
		...
		protected function parsePath($path) {
			...
			if (mb_substr($path, 0, mb_strlen(CURRENT_WORKING_DIR)) == CURRENT_WORKING_DIR) {
				$path = mb_substr($path, mb_strlen(CURRENT_WORKING_DIR) + 1);
			}
		...
		protected function readDirectory($path, $depth = 0) {
			...
			foreach ($objs as $obj) {
				if (in_array($obj, $this->ignoreNames)) {
					continue;
				}

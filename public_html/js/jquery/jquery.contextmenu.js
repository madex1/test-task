	...
	jQuery.cmenu = {
		...
		show: function (m, p, oEvent) {			/* Show menu m near parent object p	{{{ */
			...
			if (jqp.hasClass('cmenuItem') && !jqp.hasClass('cmenuItemWithSub')) {
				...
				var pm = jQuery.cmenu.getMenu(parseInt(jQuery(p.parentNode).attr('iuid'), 10));
				if (pm) {
					if (pm.sub) {
						if (pm.sub === m) {
							jQuery(prev_caller).removeClass('cmenuItemWithSub');
						} else {
							jQuery.cmenu.hideMenu(pm.sub);
			...
			var id, m, act, elem;
			m.jq.find('div.cmenuItem').each(function() {
                    elem = jQuery(this);
                    ...
                    //console.log(m.a[act])
                    if (m.a[act].onRender && typeof m.a[act].onRender === 'function') {
                       m.a[act].onRender(this); 
                    }

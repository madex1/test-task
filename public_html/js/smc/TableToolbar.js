	...
	var __drawButtons = function() {
		...
		__appendButton(btns, {
			...
			init : function(button) {
				...
				if (HandleItem !== null &&  _allow_activity) {
...
/*
	...
	this.show = function(_HandleItem, bForce) {
		...
		if (HandleItem) HandleItem.labelControl.className = 'ti';
		...
		var cpos = jQuery(container).position();


		// some tables has extra column (toggle button)
		var index = 0;
		if(HandleItem.element.childNodes[index].className=='ti-toggle') {
			index = 1;
		}
		
		var itemName    = jQuery(HandleItem.element.childNodes[index]);

		var posItemName = itemName.position();
		var posLeft     = posItemName.left + itemName.width() + cpos.left - 7 - container.scrollLeft;

		this.element.style.top = HandleItem.position.top + cpos.top + 'px';
		//this.element.style.left = container.offsetWidth - 130 + 'px';

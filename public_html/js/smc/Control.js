...
var Control = function(_oDataSet, _ItemClass, options) {
	...
	this.LabelFirstColumn = (typeof (options['label_first_column']) !== 'undefined' && typeof (options['label_first_column']) === 'string') ? options['label_first_column'] : "";
	this.PerPageLimits = typeof (options['perPageLimits']) == 'array' ? options['perPageLimits'] : new Array(10, 20, 50, 100);
	...
	var __DataSetAfterExecute = function(arrData) {
		...
		if (!error) {
			...
			switch (method.toLowerCase()) {
				...
				case "tree_move_element" : {
					...
					if (arrObjs.length && hItem && receiver) {
						...
						// hItem.parent.loaded = false;
						// hItem.parent.isExpanded = false;
						// append item in new receiver
						var itm = null;

						if (arrParams['as-sibling']) {
							// document.title = 'Move "' + hItem.name + '" in '
							// + receiver.name;
							if (before) {
								// document.title += ' before ' + before.name +
								// ' ['+before.id+']';
				...
				case 'copy_to_lang' : {
					var i, item, domainId = false;
					langId = false;
					items = hItem.control.selectedList;
					for (i in items) {
						item = items[i];
					...
					new_langId = arrParams['lang-id'];
					new_domainId = arrParams['domain-id'];

					if (new_langId == langId) {
			...
			if (error_type == '__alias__') {
				...
				var elems = jQuery(error).find("item"), count = elems.length;
				elems
						.each(function() {
							...
							html += jQuery(this).attr('path') + "<input type='text' name='new_alias" + jQuery(this).attr('id') + "'  value='" + jQuery(this)
							...
							openDialog({
							text : html,
							OKText : getLabel('js-content-alias-new'),
							...
							OKCallback : function() {
								params['alias[' + jQuery(t).attr('id') + ']'] = jQuery('#new_alias' + jQuery(t).attr('id')).val();
				...
				var callback = function() {
					hItem.control.dataSet.execute('' + method.toLowerCase() + '', params);
				};
...
Control.detectItemByMousePointer = function(x, y) {
	...
	for ( var i = 0; i < Control.instances.length; i++) {
		...
		var cpos = jQuery(Inst.initContainer).position();
...
Control.recalcSelectedItems = function(event) {
	...
	if (tn == 'textarea' || tn == 'input' || tn == 'select' || tn == 'option')
		return;
			...
			if (el.className.toLowerCase() != 'ti-toggle') {
				...
				if (event.button == 2) {
					for ( var i in Control.HandleItem.control.selectedList) {
						if (Control.HandleItem.control.selectedList[i].id == Control.HandleItem.id)
							doSelect = false;
					}
				}
				if (doSelect)
...
jQuery(document).bind('mousemove', Control.recalcSelectedItems);

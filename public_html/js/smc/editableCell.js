...
var editableCell = function(_TableItem, _Cell, _PropInfo) {
	...
	var __constructor = function() {
		...
		if (editableCell.enableEdit == false) return false;
		...
		if (editableCell.enableEdit) {
			if (!editableCell.helper) {
				...
				jQuery(editableCell.helper).css({
					position : 'absolute',
					width: '16px',
					height: '16px',
				...
				jQuery(editableCell.helper).bind('mouseover', function() {
					editableCell.hideHelper = false;
				});

				jQuery(editableCell.helper).bind('mouseout', function() {
					editableCell.handleCell = false;
					editableCell.helper.style.display = 'none';
				});

				jQuery(editableCell.helper).bind('click', function() {
					editableCell.helper.style.display = 'none';
		...
		jQuery(__self.element).bind('mouseover', function() {
			...
			var pos = jQuery(_Cell).position();
			var cpos = jQuery(Item.control.initContainer).position();
		...
		jQuery(__self.element).bind('mouseout', function(evnt) {
	...
	var _makeEditableColorField = function(value) {
		...
		var fieldHTML = '<div class="color table"><input type="text"></div>';
	...
	var __makeEditableRelationFld = function(vals, multiple) {
		...
		jQuery(relInput).add(jQuery(editControl)).keydown(function(evnt) {
	...
	var __makeEditablBooleanFld = function(val) {
		...
		jQuery(editControl).keydown(function(evnt) {
			var iKCode = evnt.keyCode;
			if (iKCode == 27) __self.restore();

...
var TableItem = function(_oControl, _oParent, _oData, _oSiblingItem, _sInsertMode) {
	...
	*/
	...
	this.appendColumn = function(fieldName) {
		...
		if (__self.isRoot) {
			...
			TableItem.setSavedOrder(fieldName, __self.control, header);
	...
	*/
	var __drawRoot = function() {
		...
		var tHead = document.createElement('thead');
		{
			...
			filterRow.className = 'filter-row';
			{
				...
				TableItem.setSavedOrder('name', __self.control, header);
		...
		if(flatMode && !objectTypesMode) {
			var exportCallback = function () {
				...
				var callback = function() {
					...
					var selectedEncoding = selectElement.item(selectedOptionIndex).text;
					var link = __self.getExportLink(null, selectedEncoding);
					window.location = link;
				};

				jQuery.get('/js/smc/html/quickExportDialog.html', function(html) {
					openDialog({
						text       : html,
						title 	   : getLabel('js-csv-export'),
						OKText     : getLabel('js-csv-export-button'),
						OKCallback : callback
					});
				});
	...
	this.callCsvImport = function (link) {
		...
		html += "<input type='file' name='csv-file' id='csv-file' size='28' />" + "<br /><br />";
		html += '<label id="csv_encoding">';
		html += 	'<span>' + getLabel('js-csv-choose-encoding') + '</span>';
		html += 	'<select name="encoding">';
		html += 		'<option value="Windows-1251">Windows-1251</option>';
...
TableItem.sortPrefix = 'sort';
TableItem.sortGlue = '|';

TableItem.orderByColumn = function(_FieldName, _Control, _ColumnEl, forcedDirection) {
	if (!_Control || !_FieldName || !_ColumnEl) return false;

	var direction = "asc";
	if (typeof forcedDirection == 'string' && forcedDirection.length > 0) {
	...
	TableItem.saveSortDirection(_FieldName, direction, true);
...
/**
 * Сохраняет запись с ключом key и значением value
 * @param {string} key имя ключа
 * @param {string} value значение
 * @returns {bool}
 * @private
 */
TableItem._saveData = function(key, value) {
...
/**
 * Возаращет значение записи по ключу key
 * @param {string} key имя ключа
 * @returns {null|string}
 * @private
 */
TableItem._getData = function(key) {
...
/**
 * Возвращщает префикс для ключа, по которому можно получить данные о сортировке поля
 * @returns {null|string}
 * @private
 */
TableItem._getSortKeyPrefix = function() {
	var data = uAdmin.data;
	...
	return [TableItem.sortPrefix, module, method].join(TableItem.sortGlue) + TableItem.sortGlue;
};

/**
 * Возвращает ключ, по которому можно получить данные о сортировке поля
 * @param {string} fieldName имя поля
 * @returns {null|string}
 * @private
 */
TableItem._getSortKey = function(fieldName) {
	return TableItem._getSortKeyPrefix() ? TableItem._getSortKeyPrefix() + fieldName : null;
};

/**
 * Возвращает направление сортировки поля
 * @param {string} fieldName имя поля
 * @returns {null|string}
 */
TableItem.getSortDirection = function(fieldName) {
	if (typeof localStorage != 'object' || typeof localStorage.getItem != 'function') {
		return null;
	}

	return TableItem._getData(TableItem._getSortKey(fieldName));
};

/**
 * Сохраняет направление сортировки для поля
 * @param {string} fieldName имя поля
 * @param {string} direction напрвление сортирвки ('asc' или 'desc')
 * @param {bool} cleanUp если true, то все сохраненные записи для текущей вкладки будут удалены
 * @returns {bool}
 */
TableItem.saveSortDirection = function(fieldName, direction, cleanUp) {
	if (typeof localStorage != 'object' || typeof localStorage.getItem != 'function') {
		return false;
	}

	var keyPrefix = TableItem._getSortKeyPrefix();
	if (cleanUp) {
		for (var key in localStorage) {
			if (localStorage.hasOwnProperty(key)) {
				var result = key.split(keyPrefix);
				var matched = (result.length === 2);
	...
	TableItem._saveData(TableItem._getSortKey(fieldName), direction);
	return true;
};

/**
 * Устанавливает ранее сохраненный порядок сортировки для поля
 * @param {string} fieldName имя поля
 * @param {Control} control экземпляр класса Control
 * @param {HTMLElement} element элемент кнопки сортировки
 */
TableItem.setSavedOrder = function(fieldName, control, element) {
	var sortDirection = TableItem.getSortDirection(fieldName);

	if (typeof sortDirection == 'string' && sortDirection.length > 0) {
		TableItem.orderByColumn(fieldName, control, element, sortDirection);
	}
};

	...
	Selection.prototype = {
		...
		getRng: function(w3c) {
			...
			if (!w3c && self.lastFocusBookmark) {
				...
				if (bookmark.startContainer && bookmark.endContainer) {
					rng = doc.createRange();
					try {
						rng.setStart(bookmark.startContainer, bookmark.startOffset || 0);
					} catch (e) {
						rng.setStart(bookmark.startContainer, 0);
					}
					try {
						rng.setEnd(bookmark.endContainer, bookmark.endOffset || 0);
					} catch (e) {
						rng.setEnd(bookmark.endContainer, 0);
					}
				} else {
					rng = bookmark;
				}
		...
		addCommands({
			...
			'Indent,Outdent': function(command) {
				...
				if (!queryCommandState('InsertUnorderedList') && !queryCommandState('InsertOrderedList')) {
					...
					if (node && node.parentNode && command == 'indent') {
						dom.setStyles(node.parentNode, {margin: "0 0 0 " + intentValue + indentUnit});
					}
				}
			},
...
define("tinymce/Editor", [
	...
	Editor.prototype = {
		...
		addButton: function(name, settings) {
			...
			if (settings.cmd && !settings.onclick) {
		...
		function bookmarkToRng(editor, bookmark) {
			...
			if (bookmark.startContainer) {
				...
				try {
					...
					rng.setStart(bookmark.startContainer);
				}
				try {
					rng.setEnd(bookmark.endContainer, bookmark.endOffset);
				} catch (e) {
					rng.setEnd(bookmark.endContainer);
				}
			} else {
				rng = bookmark;
			}

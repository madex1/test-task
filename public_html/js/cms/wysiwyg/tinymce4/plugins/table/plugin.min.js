		...
		function fillLeftDown(x, y, rows, cols) {
			...
			for (r = 1; r <= rows; r++) {
				...
				if (!tr) {
					...
					td = dom.create('td');
					tr.appendChild(td);
					dom.insertAfter(tr, grid[y][x].elm.parentNode);
				}
...
define("tinymce/tableplugin/Plugin", [
	...
	function Plugin(editor) {
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						editor.dom.setAttribs(tableElm, {
							cellspacing: addSizeSuffix(data.cellspacing),
							cellpadding: addSizeSuffix(data.cellpadding),
							border: addSizeSuffix(data.border),
						...
						var cells = dom.select('td', tableElm);
						editor.dom.setStyles(cells, {
							borderWidth: addSizeSuffix(data.border),
							padding: addSizeSuffix(data.cellpadding),
							margin: addSizeSuffix(data.cellspacing)
						});
		...
		function mergeDialog(grid, cell) {
			// umi custom:
			editor.windowManager.alert("Plase select cells you would like to merge");
			return;
			// standard logic:
			editor.windowManager.open({
				title: "Merge cells",
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						grid.merge(cell, data.cols || 1, data.rows || 1);
					});
				}
			});
		}
					...
					items: [
						{
							...
							name: 'width',
							onchange: function () {
								if (Number(this.value()) > maxCellWidth) this.value(maxCellWidth);
							}
						},
						{
							label: 'Height',
							name: 'height',
							onchange: function () {
								if (Number(this.value()) > maxCellHeight) this.value(maxCellHeight);
							}
						},
						{
							label: 'Cell type',
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						each(cells, function(cellElm) {
							...
							data.width = data.width && Number(data.width) > maxCellWidth ? maxCellWidth : data.width;
							data.height = data.height && Number(data.height) > maxCellHeight ? maxCellHeight : data.height;

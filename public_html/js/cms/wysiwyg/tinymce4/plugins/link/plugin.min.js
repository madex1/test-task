	...
	function showDialog(linkList) {
		...
		function updateText() {
			if (!initialText && data.text.length === 0) {
				...
				this.parent().parent().find('#external')[0].checked(/^http:\/\//.test(this.value()));
			}
		}
		...
		win = editor.windowManager.open({
			...
			body: [
				...
				},
				{
					...
					type: 'checkbox',
					label: 'External link',
					checked: /^http:\/\//.test(data.href),
					onclick: function () {
						var hrefInput = this.parent().parent().find("#href")[0];
						hrefInput.value(hrefInput.value().replace(/^http:\/\//, ''));
						if (this.value()) {
							hrefInput.value('http://' + hrefInput.value());
						}
					}
				},
			...
			onSubmit: function(e) {
				...
				function insertLink() {
					if (data.text == '') data.text = tinymce.translate('link');
					if (editor.selection.isCollapsed() && editor.lastSelectionBookmark) {

		...
		jQuery.ajax({
			...
			success: function(data) {
				...
				if (data.result.length < 1) {
					jQuery(jQuery(elem).find('tbody')).append('<tr><td colspan="4" style="text-align:center"><span class="runOneStat">' + getLabel('js-index-stat-nodata') + '</span></td></tr>');
				}
...
jQuery(function(){
	...
	jQuery('.tabsStat').addClass('hideTab');
	jQuery('.tabsStat').first().addClass('showTab');
	jQuery('#statLinks a').first().addClass('activeTab');
	...
	jQuery('#statLinks a').click(function(e) {
		jQuery('#statLinks a').removeClass('activeTab');
		activeTab = jQuery(this).attr('class');
		jQuery(this).addClass('activeTab');
		jQuery('.tabsStat').removeClass('showTab').addClass('hideTab');
		jQuery('#' + activeTab).removeClass('hideTab').addClass('showTab');
		if (changedDate) {
			tabsActual = {'ordersStats':false,'commonStats':false,'topPopularProduct':false};
			changedDate = false;
		}

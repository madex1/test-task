function typeControl(_typeId, _options) {
	...
	var init = function() {
		var addButton = jQuery("<span class='fg_add_group' style='height:5px;'></span>");
		...
		container.before("<div id='removeConfirm' style='display:none;' >\
							...
							<div class='eip_win_close popupClose' onclick=\"javascript:jQuery.closePopupLayer('removeConfirm'); return false;\"> </div>\
								<div class='eip_win_title'>" + getLabel("js-seo-edit-confirm_title") + "</div>\
							</div>\
							<div class='eip_win_body popupBody' onmousedown=\"jQuery('.eip_win').draggable().draggable('destroy')\">\
								<div class='popupText'>" + getLabel("js-seo-edit-confirm_text") + "</div>\
	...
	this.addGroup = function(_options) {
		if(_options.id) {
			...
			var groupContainer =
				jQuery("<div class=\"fg_container\">\
					<div class=\"fg_container_header\">\
						<span id='head"+gid+"title' class='left' style=''>" +_options.title + "</span>\
					</div>\
					<!--div class=\"group_edit\">\
						<form class='group_form'>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+gid+"title'>" + getLabel("js-island-edit-title") + "</label>\
							<input type='text' id='"+gid+"title' name='data[title]' value='" + _options.title + "' />\
						</div>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+gid+"name'>" + getLabel("js-island-edit-name") + "</label>\
					...
					<div class=\"fg_container_body\">"
						 + (_options.locked ? "" : "<span class='fg_add_field'><a href='#' class='add'>" + getLabel("js-island-edit-add_field") + "</a></span>") +
			...
			//}
			jQuery("ul", groupContainer).addBack().attr("umiGroupId", _options.id);
			...
			jQuery(".fg_container", groupContainer).sortable({connectWith : "ul.fg_container", dropOnEmpty: true, items: "li",
														 ...
														 update : function(e, ui){ 
																	...
																	var isLast      = (nextFieldId != undefined) ? "false" : ui.item.parent().attr("umiGroupId");
															 		var islandTypeId = jQuery('div[name=g_common] input.island_type:checked').val();
																	jQuery.get("/admin/seo/island_move_field_after/"+fieldId+"/"+nextFieldId+"/"+isLast+"/"+typeId+"/" + islandTypeId + "/");
																  } });
			jQuery("a.add", groupContainer).click(function() {
											_self.addField(_options.id, {id    	  : 'new',
																		 title 	  : getLabel("js-island-edit-new_field"),
																		 typeId	  : 13,
																		 typeName : '',
																		 name  	  : '',
																	     tip      : ''});
			...
			var fieldContainer =
				jQuery("<li>\
					<div class=\"view\" >\
						...
						<span id='" + fid + "control'>"
							+ (_options.locked ? "&nbsp;" :
							"<a href=\"#\" class=\"edit\" title='" + getLabel("js-island-edit-edit") + "' />\
							<a href=\"#\" class=\"remove\" title='" + getLabel("js-island-edit-remove") + "' />") +
						"</span>\
						<span id='" + fid + "save' style='display:none;'>\
							" + getLabel("js-seo-edit-saving") + "...\
						</span>\
					</div>\
					<div class=\"edit\" style=\"display:none;\">\
						<form>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+fid+"title'>" + getLabel("js-island-edit-title") + "</label>\
							<input type='text' id='"+fid+"title' name='data[title]' value=\"" + _options.title.replace('"', '&quot;') + "\" />\
						</div>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+fid+"name'>" + getLabel("js-island-edit-name") + "</label>\
							<input type='text' id='"+fid+"name' name='data[name]' value='" + _options.name + "' />\
						</div>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+fid+"type'>" + getLabel("js-island-edit-type") + "</label>\
							<select id='"+fid+"type' name='data[field_type_id]'>\
								<option value='"+_options.typeId+"'>"+_options.typeName+"</option>\
							</select>\
						</div>\
						<div style=\"width:49%;float:left;\">\
							<label for='"+fid+"restriction'>" + getLabel("js-island-edit-restriction") + "</label>\
							<select id='"+fid+"restriction' name='data[restriction_id]'></select>\
						</div>\
						<div style=\"width:49%;float:left;display:none;\" id='"+fid+"guideCont'>\
							<label for='"+fid+"guide'>" + getLabel("js-island-edit-guide") + "</label>\
						...
						<div style=\"width:49%;float:left;\">\
							...
							<label for='"+fid+"visible' class='boolean'>" + getLabel("js-island-edit-visible") + "</label>\
						</div>":"<input type='hidden' id='"+fid+"visible' name='data[is_visible]' value='1' checked='checked' />")+"\
						<div class='buttons'>\
							<div><input type='button' value='"+getLabel("js-island-confirm-cancel")+"' class='cancel button' />\
							<span class='l' /><span class='r' /></div>\
							<div><input type='button' value='"+getLabel("js-island-save-edit-field")+"' class='ok button' />\
							<span class='l' /><span class='r' /></div>\
						</div>\
						</form>\
					</div>\
			...
			jQuery("a.remove", fieldContainer).click(
					function () {
						...
						jQuery("input.RemoveConfirmYes").click(function() {
							...
							fieldContainer.remove();
							jQuery.get("/admin/seo/island_field_delete/" + _options.id + "/" + typeId + "/" + jQuery('div[name=g_common] input.island_type:checked').val() + "/.xml?noredirect=true");
							jQuery.closePopupLayer('removeConfirm');
							return false;
						});
						return false;
	...
	var saveField = function(id, context) {
		...
		param += '&csrf=' + csrfProtection.getToken();
		param += '&currentIslandType=' + jQuery('div[name=g_common] input.island_type:checked').val();

		if(id == 'new') {
			jQuery
				.post("/admin/seo/island_field_add/" + typeId + "/do/.xml?noredirect=true",
				   param,
				   function(data) {
					   ...
					   if (errs.length) {
						   jQuery.jGrowl(errs[0].firstChild.nodeValue, { header: getLabel('js-error-header') });
					   }

					   jQuery("*[id]" , context).each( function() { this.id = this.id.replace(/new/, newFieldId); } );
					   if (!errs.length) jQuery("#f" + newFieldId + "control", context).show();
			...
			jQuery
				.post("/admin/seo/island_field_edit/" + id + "/" + typeId + "/" + jQuery('div[name=g_common] input.island_type:checked').val() +  "/do/.xml?noredirect=true",
				   param,
				   function(data) {
					   var errs = data.getElementsByTagName("error");
					   if (errs.length) {
					   ...
					   if (!errs.length) jQuery("#f" + id + "control", context).show();
...
jQuery(document).ready(function() {
	jQuery('input.island_type').bind('change', function () {
		jQuery('div.island_type').css('display', 'none');
		jQuery('div.island_type#g_' + jQuery(this).attr('value') + '').css('display', '');
	});
});

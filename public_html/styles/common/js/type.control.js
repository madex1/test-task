function typeControl(_typeId, _options) {
				...
				jQuery("<li>\
					...
					<div class=\"edit\" style=\"display:none;\">\
						...
						<div style=\"width:49%;float:left;\">\
							...
							<input type='text' id='"+fid+"title' name='data[title]' value=\"" + _options.title.replace(/"/g, '&quot;') + "\" />\
						</div>\
						<div style=\"width:49%;float:left;\">\
						...
						<div style=\"width:49%;float:left;\">\
							...
							<input type='text' id='"+fid+"tip' name='data[tip]' value=\"" + _options.tip.replace(/"/g, '&quot;') + "\" />\
						</div>\
						<div style=\"width:49%;float:left;\">\
	...
	var saveField = function(id, context) {
		...
		fid = "#headf" + id;
		jQuery(fid + "title", context).html(field.title+(field.required=='checked' ? " *" : ""));
		...
		if(id == 'new') {
			function saveNewField(ignoreExisting) {
				jQuery
					.post("/admin/" + (formMode ? "webforms" : "data") + "/type_field_add/" + field.groupId + "/" + typeId + "/do/.xml?noredirect=true",
					param += ignoreExisting ? '&ignoreChildGroup=true' : '',
					function(data) {
				...
				if(ignoreExisting) {
					closeDialog("saveFieldConfirm");
				}
			}
			jQuery.post("/admin/" + (formMode ? "webforms" : "data") + "/isFieldExist/.json", {
					"group-id" : field.groupId,
					"type-id" : typeId,
					"csrf" : csrfProtection.getToken()
				}, function(data) {
					if(data.data.isExist == 'false') {
						var groupName = jQuery('#g' + field.groupId + 'title').val(),
						html = getLabel('js-add-field-part-one') + groupName + getLabel('js-add-field-part-two') + groupName + getLabel('js-add-field-part-three');
						html += "<div class='eip_buttons'><input id='js-data-add-field' type='button' class='ok' value='"+getLabel('js-data-add-field')+"' />";
						html += "<input id='js-data-add-field-cancel' type='button' class='back' value='"+getLabel('js-cancel') + "' />";
						html += "<div style='clear: both;' /></div>";
						openDialog({
							name : 'saveFieldConfirm',
							width : 390,
							stdButtons : false,
							text : html,
							title : getLabel('js-add-field-header')
						});
						jQuery('#js-data-add-field-cancel').bind('click', function() {
							closeDialog("saveFieldConfirm");
							jQuery('li[umifieldid^="new"]').remove();
						});
						jQuery('#js-data-add-field').bind('click', function() {
							saveNewField(true)
						});
					} else {
						saveNewField(false);
			...
			jQuery
				.post("/admin/" + (formMode ? "webforms" : "data") + "/type_field_edit/" + id + "/" + typeId + "/do/.xml?noredirect=true",
				   param,
				   function(data) {
					   var errs = data.getElementsByTagName("error");
					   if (errs.length) {

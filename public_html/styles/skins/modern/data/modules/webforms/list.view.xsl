...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<form class="filter-container" action="" method="post"
				...
				<div class="layout-row-icon">
					<div class="layout-col-control">
						<select name="form_id" class="default newselect" placeholder="Выберите форму">
							<option value=""></option>
							<xsl:apply-templates select="items/item"/>
						</select>
					</div>
					<div class="layout-col-icon">
						<input type="submit" class="btn color-blue btn-small fcApplyButton" value="Фильтровать"/>
					</div>
				</div>
			</form>
		</div>
	...
	<xsl:template match="/result[@method = 'forms']/data[@type = 'list' and @action = 'view']">
		...
		<div class="location" xmlns:umi="http://www.umi-cms.ru/TR/umi">
			<div class="imgButtonWrapper loc-left">
				<a id="addForms" class="btn color-blue" href="{$lang-prefix}/admin/&sys-module;/form_add/{basetype/@id}">&label-type-add;</a>
			</div>
			<xsl:call-template name="entities.help.button" />
		</div>

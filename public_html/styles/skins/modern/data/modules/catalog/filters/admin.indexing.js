 ...
 *
 * Глобальные объекты (функции): jQuery, openDialog, oTable
 */
...
var AdminIndexing = (function($){
	...
	/** Инициализация настроек (объекта Settings) */
	function init() {
		 ...
		 */
		Settings.module = {
			dirPath:  '/styles/skins/modern/data/modules/catalog/filters/',
			storage: {
			...
			loadingOverlay: {
				image: '/styles/skins/modern/data/modules/catalog/filters/loadingoverlay/img/ajax_loader_gray_128.gif',
		...
		Settings.popups = {
			catalog: {
				...
				changingTitle: 'Изменить раздел индексирования',
				width: '100%',
				height: 335,
				resource: 'tree.html?hierarchy_types=catalog-category,content-page',
		...
		(function () {
			...
			for (var buttonName in Settings.buttons) {
				if (Settings.buttons.hasOwnProperty(buttonName)) {
					button = Settings.buttons[buttonName];
		...
		Settings.table =  {
			...
			rows: {
				menu: [
					['delete', 'i-remove', Controller.cancelChoosing]
				]
			}
		};
	...
	Controller._deleteIndexes = function(elements) {
		...
		openDialog('', confirmSettings.title, {
	...
	Controller.showMessage = function(message) {
		if (typeof $.jGrowl === 'function' && Settings.module.messagesOptions.enabled) {
	...
	Controller.onRenderComplete = function() {
		...
		var $namesColumnCells = $('tr[rel] td.table-cell:first-child', Settings.table.object.container);
		var $namesLinks = $namesColumnCells.find('.name_col');
	...
	function makePostRequest(url, data, callback, isHTML) {
		...
		$.ajax({
			...
			error: function(jqXHR) {
				...
				message = message || getLabel('js-server_error');

				if (jqXHR.status === 403 && jqXHR.responseJSON && jqXHR.responseJSON.data && jqXHR.responseJSON.data.error) {
					message = jqXHR.responseJSON.data.error;
				}

		...
		child.animate({
			...
			jQuery('#' + typeId +'-child .tableDashboard .events_header').after([
				'<div class="events_header">',
					...
					'<div class="buttons">',
						'<div>',
							'<input type="button" class="btn color-blue btn-smol" id="mark_read" value="' + getLabel('js-dashboard-mark-read') + '" disabled="disabled" onclick="javascript:markReadEvents();"/>',
						'</div>',
					'</div>',
				'</div>'
			].join(''));

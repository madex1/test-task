...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="data">
		<p class="error">
			<strong>
				<xsl:text>&styles-skins-eip-main-error;: </xsl:text>
			</strong>
			<xsl:text>&styles-skins-eip-main-cannot_view_current_page_type; "</xsl:text>
			<strong>
				<xsl:value-of select="concat($data-type, '.', $data-action)" />
			</strong>
			<xsl:text>" &#8212; &styles-skins-eip-main-have_not_learned;</xsl:text>

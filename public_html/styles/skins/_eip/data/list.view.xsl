...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="data[@type = 'list']">
		<div id="eip_page">
			<h3>
				<xsl:text>&js-eip-create-page;</xsl:text>
			</h3>
	...
	<xsl:template match="hierarchy-type">
		<div class="eip_page_type_choice">
			<xsl:choose>
				<xsl:when test="count(document(concat('udata://system/getObjectTypesList/', @module, '/', @method, '/', ../../@domain-id))/udata/items/item) &gt; 1">
					<xsl:attribute name="onclick">showSubtypes(this, '<xsl:value-of select="@module" />_<xsl:value-of select="@method" />');</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="hierarchy-type" mode="subtypes">
		<xsl:apply-templates select="document(concat('udata://system/getObjectTypesList/', @module, '/', @method, '/', ../../@domain-id))/udata">
			<xsl:with-param name="id" select="@id" />
			<xsl:with-param name="module" select="@module" />
			<xsl:with-param name="method" select="@method" />
			<xsl:with-param name="csrf"><xsl:value-of select="//@csrf" /></xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template match="udata[@method = 'getObjectTypesList' and count(//item) > 1]">
		...
		<xsl:param name="csrf" />
		<form id="eip_page_subtype_{$module}_{$method}" method="get" style="display:none;">
			<xsl:apply-templates select="//item">
				<xsl:with-param name="hierarchy-type-id" select="$id" />
			</xsl:apply-templates>
			<div class="eip_buttons">
				<input type="submit" value="&label-proceed;" class="primary ok" />
				<input type="button" value="&label-cancel;" class="back" onclick="hideSubtypes(this.form);" />
				<div style="clear:both;"/>
			</div>
			<input type="hidden" name="hierarchy-type-id" value="{$id}" />

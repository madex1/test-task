...
<xsl:stylesheet version="1.0"
		...
		<script	type="text/javascript">
			...
			$(document).ready(function() {
				...
				<xsl:if test="$template &gt; 0">
					oDefaultFilter.setTemplate('<xsl:value-of select="$template"/>');
				</xsl:if>
				...
				<xsl:choose>
					...
					<xsl:when test="$module = 'catalog' and $method = 'tree'"><![CDATA[
						var menu = [
							...
							['change-parent', false, ContextMenu.itemHandlers.changeParent],
	...
	<xsl:template name="std-form-is-visible">
		...
		<div class="field">
			<label class="inline" for="is-visible_{generate-id()}">
				<span class="label">
					<input type="hidden" name="is-visible" value="0" />
					<input type="checkbox" name="is-visible" value="1" class="checkbox" id="is-visible_{generate-id()}">
						<xsl:if test="$value">
							<xsl:attribute name="checked">checked</xsl:attribute>
						</xsl:if>
					</input>
	...
	<xsl:template name="std-form-is-default">
		...
		<div class="field">
			<label class="inline" for="is-default_{generate-id()}">
				<span class="label">
					<input type="hidden" name="is-default" value="0" />
					<input type="checkbox" name="is-default" value="1" class="checkbox" id="is-default_{generate-id()}">
						<xsl:if test="$value">
							<xsl:attribute name="checked">checked</xsl:attribute>
						</xsl:if>
					</input>
				...
				<div class="field">
					<label>
						<span class="label">
							...
							<a href="/admin/data/type_edit/{$value}" id="edit">&label-edit-type;</a>
						</span>
						<span>
				...
				<div class="field">
					<label>
						<span class="label_edit">
							<a href="/admin/data/type_edit/{$value}" id="edit" name="edit">&label-edit-type;</a>
						</span>
						<span>

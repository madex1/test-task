...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="chart" mode="yandex">
		<a href="#" id="{@name}"><img src="/images/cms/admin/mac/{@name}.png" alt="{@name}"/></a>
	</xsl:template>
	...
	<xsl:template match="filter" mode="yandex">
		...
		<a href="/admin/stat/view_counter/{$section}/{$report}/{$counter}/?filter={@name}" id="ym_{@name}" onclick="drawChart('{$section}', '{$report}', '{@name}', '{$counter}', '{$mainColumn}', '{$chartType}'); return false;" />
		<script>
			jQuery('#ym_<xsl:value-of select="@name" />').html(getLabel('js-label-counter-<xsl:value-of select="@name" />'));
		</script>

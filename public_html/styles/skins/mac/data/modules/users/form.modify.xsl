...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umi="http://www.umi-cms.ru/TR/umi">
	...
	<xsl:template match="/result[@module = 'users' and @method = 'add']//field[@type = 'password']" mode="form-modify">
		<div class="field">
			<label for="{generate-id()}">
				...
				<span>
					<input type="password" name="{@input_name}" value="{.}" id="{generate-id()}" class="required password" />
				</span>
			</label>
		</div>
	</xsl:template>
	...
	<xsl:template match="properties/group[@name = 'more_info']" mode="form-modify">
		...
		<div class="panel properties-group" name="g_{@name}">
			...
			<div class="content">
				<xsl:apply-templates select="/result[@module = 'users' and @method = 'edit']" mode="form-modify-actAsUser" />

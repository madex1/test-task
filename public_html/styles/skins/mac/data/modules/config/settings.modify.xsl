...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/result[@method = 'security']/data/group[@name='security-audit']" mode="settings.modify" >
		<script type="text/javascript" src="/styles/common/js/security.js"></script>
		<table class="tableContent" id="testsTable">
			<thead>
				<th colspan="2">
					<xsl:value-of select="@label" />
				</th>
		...
		<div class="buttons">
			<div>
				<input type="button" id="startSecurityTests" value="&js-check-security;"/><span class="l"></span><span class="r"></span>
			</div>
		</div>
	</xsl:template>

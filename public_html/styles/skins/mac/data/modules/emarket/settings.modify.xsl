...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
				...
				<![CDATA[
					jQuery(function() {
						...
						jQuery(".emarket_yandex_market_config_btn[rel]").click(function() {
							...
							jQuery(this).parents('.tableContent').find('input').each(function() {
						...
						jQuery('#popupLayer_createTokenPopup #createTokenFormCancel').live('click', function() {
							jQuery.closePopupLayer('createTokenPopup');
						});
						// Обработка продолжения генерации
						jQuery('#popupLayer_createTokenPopup #createTokenFormSend').live('click', function() {
							jQuery('#createTokenForm').submit();
						});
					});
				]]>
	...
	<xsl:template match="/result[@method = 'yandex_market_config']//group" mode="settings-modify">
		...
		<div class="panel properties-group">
			<div class="header">
				<span>&group-settings-for;<xsl:value-of select="@name" /></span>
			...
			<div class="content">
				<table class="tableContent">
					<tbody>
						<tr>
							<td><label for="{$client-id/@name}"><xsl:text>&option-clientId;</xsl:text></label></td>
							<td colspan="2"><input type="text" name="{$client-id/@name}" value="{$client-id/value}" id="{$client-id/@name}" /></td>
						</tr>
						<tr>
							<td class="eq-col"><label for="{$client-password/@name}"><xsl:text>&option-clientSecret;</xsl:text></label></td>
							<td colspan="2"><input type="text" name="{$client-password/@name}" value="{$client-password/value}" id="{$client-password/@name}" /></td>
						</tr>
						<tr>
							<td class="eq-col"><label for="{$client-token/@name}"><xsl:text>&option-token;</xsl:text></label></td>
							<td><input type="text" name="{$client-token/@name}" value="{$client-token/value}" id="{$client-token/@name}" /></td>
							<td class="token_button"><a target="_blank" class="emarket_yandex_market_config_btn" rel="{@name}" href="javascript:void();">Получить&nbsp;токен</a></td>
						</tr>
						<tr>
							<td class="eq-col"><label for="{$client-login/@name}"><xsl:text>&option-login;</xsl:text></label></td>
							<td colspan="2"><input type="text" name="{$client-login/@name}" value="{$client-login/value}" id="{$client-login/@name}" /></td>
						</tr>
						<tr>
							<td class="eq-col"><label for="{$client-market-token/@name}"><xsl:text>&option-marketToken;</xsl:text></label></td>
							<td colspan="2"><input type="text" name="{$client-market-token/@name}" value="{$client-market-token/value}" id="{$client-market-token/@name}" /></td>
						</tr>
						<tr>
							<td class="eq-col"><label for="{$client-campaign-id/@name}"><xsl:text>&option-marketCampaignId;</xsl:text></label></td>
							<td colspan="2"><input type="text" name="{$client-campaign-id/@name}" value="{$client-campaign-id/value}" id="{$client-campaign-id/@name}" /></td>
						</tr>
						<tr>
							...
							<td colspan="2">
								...
								<input type="checkbox" name="{$cash-on-delivery/@name}" value="1" id="{$cash-on-delivery/@name}" class="check">
						...
						<tr>
							...
							<td colspan="2">
								...
								<input type="checkbox" name="{$card-on-delivery/@name}" value="1" id="{$card-on-delivery/@name}" class="check">
						...
						<tr>
							...
							<td colspan="2">
								...
								<input type="checkbox" name="{$shop-prepaid/@name}" value="1" id="{$shop-prepaid/@name}" class="check">
			...
			<div class="content">
				...
				<script type="text/javascript">
					...
					function rebuildTopItems() {
						...
						openDialog({
							'title': getLabel('js-emarket-reindex-header'),
							'text': '<span id="processReindex">' + getLabel('js-emarket-reindex') + '</span>' + '<p id="emarket-reindex-log" />',
						});
				...
				<div class="buttons emarket_config_btn" id="rebuildTopItems" style="float:right">
					<div>
						<input type="button" value="&order-reindex;"/><span class="l"></span><span class="r"></span>
						<span class="l"></span>
						<span class="r"></span>
					</div>
				</div>

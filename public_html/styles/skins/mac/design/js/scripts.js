...
jQuery(document).ready(function() {
	...
	var toggleMenuVisibility = function() {
		...
		if (!el.hasClass('act')) {
			...
			$('#dock').after('<div id="scope"> </div>');
			jQuery('#scope').bind('click', {}, toggleMenuVisibility );
			Control.enabled = false;
		} else {
			jQuery('div:first', el).hide(0);
			el.removeClass('act');
			jQuery('#scope').unbind('click', toggleMenuVisibility ).remove();
	...
	//----------------------------------------------------
	// Toggle fields
	var toggleFieldsContainer = jQuery("div.toggle_fields");
	jQuery("a.toggle_fields_expander").click(function(){
		if(!getCookie("toggleExtendedFields")||toggleFieldsContainer.css("display")=="none") {
			toggleFieldsContainer.css("display","");
		} else {
			toggleFieldsContainer.css("display","none");
		}
		if(toggleFieldsContainer.css("display") == "none") {
			setCookie("toggleExtendedFields", null);
		} else {
			setCookie("toggleExtendedFields", "1");
		}
	});
	if(getCookie("toggleExtendedFields")) {
		toggleFieldsContainer.css("display","");
	} else {
		toggleFieldsContainer.css("display","none");
	}
	...
	var showelfinderFileBrowser = function(folder, folder_hash, lang) {
		...
		jQuery('#popupLayer_Filemanager .popupBody').append(options);
	};
	// Date
...
jQuery(window).ready(function(){
	if(jQuery('form div.panel:first')){
		jQuery('form div.panel:first').find('div.text:first').css('width', '49.7%').css('margin', '0').css('padding','0').css('float', 'left');
...
jQuery(document).ready(function(){
	...
	if (uAdmin.data.module != 'events') {
		jQuery.getJSON('/admin/events/feed/.json', {'filter':'users-adv-message', 'onlyUnread':'true', 'limit':1}, function(r) {
